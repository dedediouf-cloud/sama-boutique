...
export default function SuperAdminDashboard() {
  ...
  const changerMonMotDePasse = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        // Message explicite si l'API n'est pas déployée correctement
        if (res.status === 404 || res.status === 405) {
          setPasswordError(
            "L'API de changement de mot de passe n'est pas disponible sur ce déploiement. " +
              "Vérifie que le fichier src/app/api/superadmin/account/route.ts est bien présent, puis redéploie."
          );
